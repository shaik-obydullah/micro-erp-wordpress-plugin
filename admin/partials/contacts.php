...
$editing = null;
if ( $edit_id ) {
	$editing = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}oby_mi_erp_contacts WHERE id = %d", $edit_id ) );
}

$type_filter = oby_mi_erp_query_key( 'type' );
...
$back_url = oby_mi_erp_admin_url( 'contacts' );
?>
<div class="wrap oby-mi-erp-page">
	<h1 class="wp-heading-inline mb-3">
		<?php echo $editing ? esc_html__( 'Edit Contact', 'obydullah-micro-erp' ) : esc_html__( 'Contacts', 'obydullah-micro-erp' ); ?>
		<?php if ( ! $editing ) : ?>
			...
			<div class="col-lg-6 col-md-12">
				<div class="bg-light p-4 rounded shadow-sm">
					<h2 id="form-title" class="mb-3 mt-1">
						<?php echo $editing ? esc_html__( 'Contact Information', 'obydullah-micro-erp' ) : esc_html__( 'New Contact', 'obydullah-micro-erp' ); ?>
					</h2>
				...
				<div class="bg-light p-3 rounded shadow-sm border">
					...
					<form method="get" action="" class="search-section mb-3">
						...
						<div class="search-toolbar d-flex flex-wrap align-items-center gap-2">
							...
							<input type="text" name="s" id="contact-search" class="form-control form-control-sm search-field" placeholder="<?php esc_attr_e( 'Search contacts...', 'obydullah-micro-erp' ); ?>" value="<?php echo esc_attr( $search ); ?>">
							...
							$all_url = oby_mi_erp_admin_url( 'contacts', $pill_args );
							?>
							<div class="filter-pills ml-auto" role="group" aria-label="<?php esc_attr_e( 'Filter by contact type', 'obydullah-micro-erp' ); ?>">
								<a href="<?php echo esc_url( $all_url ); ?>" class="<?php echo esc_attr( '' === $type_filter ? 'active' : '' ); ?>"><?php esc_html_e( 'All Types', 'obydullah-micro-erp' ); ?></a>
								<?php foreach ( array( 'customer', 'vendor', 'supplier' ) as $t ) : ?>
									<a href="<?php echo esc_url( oby_mi_erp_admin_url( 'contacts', array_merge( $pill_args, array( 'type' => $t ) ) ) ); ?>" class="<?php echo esc_attr( $type_filter === $t ? 'active' : '' ); ?>"><?php echo esc_html( ucfirst( $t ) ); ?></a>
								<?php endforeach; ?>
							</div>
						</div>
					</form>
							...
							<tbody class="bg-white">
								...
								<?php foreach ( $rows as $row ) : ?>
									<tr>
										...
										<td><?php echo oby_mi_erp_contact_type_badge( $row->type ); // phpcs:ignore WordPress.Security.EscapeOutput ?></td>
										...
										<td>
											<div class="pos-row-actions">
												<a href="<?php echo esc_url( oby_mi_erp_admin_url( 'contacts', array( 'edit' => $row->id ) ) ); ?>" class="pos-action edit pos-icon" aria-label="<?php esc_attr_e( 'Edit', 'obydullah-micro-erp' ); ?>" title="<?php esc_attr_e( 'Edit', 'obydullah-micro-erp' ); ?>"><span class="dashicons dashicons-edit" aria-hidden="true"></span></a>
					...
					<?php oby_mi_erp_render_pagination( 'contacts', $total_items, $per_page ); ?>
