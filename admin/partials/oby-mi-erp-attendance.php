...
if ( $oby_mi_erp_employees ) {
	...
	$oby_mi_erp_att            = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}oby_mi_erp_attendance WHERE date = %s AND employee_id IN ({$oby_mi_erp_in_placeholders})", array_merge( array( $oby_mi_erp_date ), $oby_mi_erp_ids ) ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare -- derived value recomputed per request; IN-clause placeholders generated by array_fill count, values passed as array args.
