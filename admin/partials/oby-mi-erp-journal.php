...
if ( ! empty( $oby_mi_erp_entries ) ) {
	...
	$oby_mi_erp_lines           = $wpdb->get_results( $wpdb->prepare( "SELECT l.*, a.code, a.name FROM {$wpdb->prefix}oby_mi_erp_journal_lines l INNER JOIN {$wpdb->prefix}oby_mi_erp_accounts a ON a.id = l.account_id WHERE l.entry_id IN ({$oby_mi_erp_in_placeholders}) ORDER BY l.id ASC", $oby_mi_erp_ids ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare -- $oby_mi_erp_in_placeholders is a dynamically built %d list matched 1:1 with $oby_mi_erp_ids, passed as the array form of $wpdb->prepare().
					...
					<div class="table-responsive">
						<table class="table table-striped table-hover table-bordered mb-2">
							...
							<tbody class="bg-white">
								...
								<?php endif; ?>
								<?php
								foreach ( $oby_mi_erp_entries as $oby_mi_erp_entry ) :
