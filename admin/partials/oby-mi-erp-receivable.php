...
$back_url = oby_mi_erp_admin_url( 'receivable' );
?>
<div class="wrap oby-mi-erp-page">
	...
	<div class="row mt-3">
		<div class="col-lg-12">
			<?php oby_mi_erp_render_search_bar( 'receivable', __( 'Search Receivables', 'obydullah-micro-erp' ), __( 'Search by customer or invoice #...', 'obydullah-micro-erp' ), array(), $search ); ?>
		</div>
	</div>
	...
	<div class="row mt-1">
		<div class="col-lg-12">
			<div class="bg-light p-3 rounded shadow-sm border">
						...
						<tbody class="bg-white">
							...
							<?php foreach ( $rows as $row ) :
								...
								<tr>
									...
									<td class="text-right"><?php echo esc_html( oby_mi_erp_format_money( $row->total ) ); ?></td>
									<td class="text-right"><?php echo esc_html( oby_mi_erp_format_money( $row->amount_paid ) ); ?></td>
									<td class="text-right"><strong<?php echo $overdue ? ' style="color:#d63638;"' : ''; ?>><?php echo esc_html( oby_mi_erp_format_money( $balance ) ); ?></strong></td>
									<td><?php echo oby_mi_erp_status_badge( $row->payment_status ); // phpcs:ignore WordPress.Security.EscapeOutput ?></td>
									<td class="text-right">
										<div class="pos-row-actions">
											<a href="<?php echo esc_url( oby_mi_erp_admin_url( 'sales', array( 'edit' => $row->id ) ) ); ?>" class="pos-action edit pos-icon" aria-label="<?php esc_attr_e( 'View', 'obydullah-micro-erp' ); ?>" title="<?php esc_attr_e( 'View', 'obydullah-micro-erp' ); ?>"><span class="dashicons dashicons-visibility" aria-hidden="true"></span></a>
											<a href="<?php echo esc_url( oby_mi_erp_admin_url( 'sales', array( 'pay' => $row->id ) ) ); ?>" class="pos-action pay"><?php esc_html_e( 'Record Payment', 'obydullah-micro-erp' ); ?></a>
										</div>
									</td>
								</tr>
							<?php endforeach; ?>
							<tr class="total-row">
								...
								<td class="text-right"><strong><?php echo esc_html( oby_mi_erp_format_money( $total_original ) ); ?></strong></td>
								<td class="text-right"><strong><?php echo esc_html( oby_mi_erp_format_money( $total_paid ) ); ?></strong></td>
								<td class="text-right"><strong><?php echo esc_html( oby_mi_erp_format_money( $total_balance ) ); ?></strong></td>
								<td></td>
								<td></td>
							</tr>
						</tbody>
				...
				<?php oby_mi_erp_render_pagination( 'receivable', $total_items, $per_page ); ?>
