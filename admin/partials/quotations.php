...
$edit_items = array();
if ( $edit_id ) {
	$editing   = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}oby_mi_erp_quotations WHERE id = %d", $edit_id ) );
	$edit_items = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}oby_mi_erp_quotation_items WHERE quotation_id = %d ORDER BY id ASC", $edit_id ) );
}
...
$back_url = oby_mi_erp_admin_url( 'quotations' );

oby_mi_erp_print_admin_notice();
?>
<div class="wrap oby-mi-erp-page">
	<h1 class="wp-heading-inline mb-3">
		<?php echo $editing ? esc_html__( 'Edit Quotation', 'obydullah-micro-erp' ) : esc_html__( 'Quotations', 'obydullah-micro-erp' ); ?>
		<?php if ( ! $editing ) : ?>
				...
				<div class="col-lg-6 col-md-12">
					<div class="bg-light p-4 rounded shadow-sm mb-4">
						...
						<div class="mb-3">
							...
							<input type="text" class="form-control" value="<?php echo $editing ? esc_attr( $editing->quotation_no ) : esc_attr( oby_mi_erp_next_quotation_no() ); ?>" readonly style="background:#f9f9f9;">
						</div>
	...
	<div class="row mt-3">
		<div class="col-lg-12">
			<?php oby_mi_erp_render_search_bar( 'quotations', __( 'Search Quotations', 'obydullah-micro-erp' ), __( 'Search by quotation # or customer...', 'obydullah-micro-erp' ), array(), $search ); ?>
		</div>
	</div>
	...
	<div class="row mt-1">
		<div class="col-lg-12">
				<div class="bg-light p-3 rounded shadow-sm border">
							...
							<tbody class="bg-white">
								...
								<?php foreach ( $rows as $q ) : ?>
									<tr>
										...
										<td><?php echo esc_html( oby_mi_erp_contact_name( $q->contact_id ) ); ?></td>
										<td><?php echo esc_html( $q->quotation_date ); ?></td>
										<td><?php echo esc_html( $q->valid_until ); ?></td>
										<td class="text-right fw-bold"><?php echo esc_html( oby_mi_erp_format_money( $q->total ) ); ?></td>
										<td><?php echo oby_mi_erp_status_badge( $q->status ); // phpcs:ignore WordPress.Security.EscapeOutput ?></td>
										<td>
											<div class="pos-row-actions">
												<a href="<?php echo esc_url( oby_mi_erp_admin_url( 'quotations', array( 'edit' => $q->id ) ) ); ?>" class="pos-action edit pos-icon" aria-label="<?php esc_attr_e( 'Edit', 'obydullah-micro-erp' ); ?>" title="<?php esc_attr_e( 'Edit', 'obydullah-micro-erp' ); ?>"><span class="dashicons dashicons-edit" aria-hidden="true"></span></a>
					...
					<?php oby_mi_erp_render_pagination( 'quotations', $total_items, $per_page ); ?>
