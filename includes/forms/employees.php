...
function micro_erp_handle_employee_form( $action ) {
	...
	if ( ! $data['employee_id'] || ! $data['name'] ) {
		micro_erp_redirect_notice( __( 'Employee ID and name are required.', 'micro-erp' ), 'error' );
		return;
	}
	...
	if ( $exists ) {
		micro_erp_redirect_notice( __( 'An employee with that ID already exists.', 'micro-erp' ), 'error' );
		return;
	}
	...
	if ( 'update_employee' === $action ) {
		...
		$entity_id = $id;
		$message   = __( 'Employee updated.', 'micro-erp' );
	} else {
		$wpdb->insert( $table, $data );
		$entity_id = (int) $wpdb->insert_id;
		$message   = __( 'Employee created.', 'micro-erp' );
	}
...
function micro_erp_handle_delete_employee() {
	...
	global $wpdb;
	$wpdb->delete( micro_erp_table( 'employees' ), array( 'id' => $id ) );
	micro_erp_audit_log( 'delete', 'employee', $id, 'Deleted employee #' . $id );
	micro_erp_redirect_notice( __( 'Employee deleted.', 'micro-erp' ) );
}
