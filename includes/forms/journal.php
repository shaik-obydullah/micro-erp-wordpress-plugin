...
function micro_erp_handle_journal_form() {
	micro_erp_verify_nonce( 'micro_erp_journal_save' );
...
function micro_erp_handle_transaction_form() {
	...
	if ( 'expense' === $mode ) {
		$acct  = $account_id ? $account_id : micro_erp_default_account( 'expense', '5001' );
		$lines = array(
			array( 'account_id' => $acct, 'debit' => $amount, 'credit' => 0 ),
			array( 'account_id' => micro_erp_default_account( 'asset', '1001' ), 'debit' => 0, 'credit' => $amount ),
		);
		$ref_type = 'expense';
	} else {
		$acct  = $account_id ? $account_id : micro_erp_default_account( 'income', '4001' );
		$lines = array(
			array( 'account_id' => micro_erp_default_account( 'asset', '1001' ), 'debit' => $amount, 'credit' => 0 ),
			array( 'account_id' => $acct, 'debit' => 0, 'credit' => $amount ),
		);
		$ref_type = 'income';
	}

	$entry_id = micro_erp_create_journal_entry( $date, $description, $lines, $ref_type, 0 );
...
function micro_erp_handle_delete_journal() {
	micro_erp_verify_nonce( 'micro_erp_journal_delete' );
