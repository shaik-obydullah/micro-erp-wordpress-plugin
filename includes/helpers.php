...
function li_mi_erp_table( $name ) {
	global $wpdb;
	return $wpdb->prefix . LI_MI_ERP_TABLE . $name;
}
 ...
 */
function li_mi_erp_admin_url( $page, $args = array() ) {
...
function li_mi_erp_get_currency_symbol() {
	return get_option( 'li_mi_erp_currency_symbol', '$' );
}

function li_mi_erp_format_money( $amount ) {
	$symbol = li_mi_erp_get_currency_symbol();
	$value  = number_format( (float) $amount, 2 );
	return $symbol . $value;
}

function li_mi_erp_get_active_fiscal_year() {
	global $wpdb;
...
function li_mi_erp_get_fiscal_year_id() {
	$fy = li_mi_erp_get_active_fiscal_year();
	return $fy ? (int) $fy->id : 0;
}

function li_mi_erp_get_setting( $key, $default = '' ) {
	global $wpdb;
...
function li_mi_erp_set_setting( $key, $value ) {
	global $wpdb;
	$table = li_mi_erp_table( 'settings' );
...
function li_mi_erp_audit_log( $action, $entity_type, $entity_id, $description = '' ) {
	global $wpdb;
...
function li_mi_erp_next_employee_id() {
	global $wpdb;
...
function li_mi_erp_next_number( $table, $column, $prefix ) {
	global $wpdb;
	$year = current_time( 'Y' );
...
function li_mi_erp_next_quotation_no() {
	return li_mi_erp_next_number( li_mi_erp_table( 'quotations' ), 'quotation_no', 'QUO-' );
}

function li_mi_erp_next_sale_no() {
	return li_mi_erp_next_number( li_mi_erp_table( 'sales' ), 'sale_no', 'SALE-' );
}

function li_mi_erp_redirect_notice( $message, $type = 'success' ) {
	set_transient( 'li_mi_erp_admin_notice', array( 'message' => $message, 'type' => $type ), 30 );
}

function li_mi_erp_print_admin_notice() {
	$notice = get_transient( 'li_mi_erp_admin_notice' );
	if ( $notice ) {
		delete_transient( 'li_mi_erp_admin_notice' );
		printf(
...
function li_mi_erp_verify_nonce( $action, $arg = '_wpnonce' ) {
...
function li_mi_erp_get_accounts( $type = '' ) {
	global $wpdb;
	if ( $type ) {
...
function li_mi_erp_account_balance( $account_id ) {
	global $wpdb;
	$account_id = (int) $account_id;
	$table      = li_mi_erp_table( 'journal_lines' );
...
function li_mi_erp_total_income() {
	global $wpdb;
...
function li_mi_erp_total_expense() {
	global $wpdb;
	return (float) $wpdb->get_var(
		$wpdb->prepare(
...
function li_mi_erp_status_badge( $status ) {
	...
	return li_mi_erp_badge( $status, $map );
}

function li_mi_erp_contact_type_badge( $type ) {
	$map = array(
		'customer' => 'status-info',
		'vendor'   => 'status-neutral',
		'supplier' => 'status-warning',
	);
	return li_mi_erp_badge( $type, $map );
}

function li_mi_erp_create_journal_entry( $date, $description, $lines, $reference_type = null, $reference_id = null ) {
	global $wpdb;

	$fiscal_year_id = li_mi_erp_get_fiscal_year_id();

	$wpdb->insert(
		li_mi_erp_table( 'journal_entries' ),
		array(
			'entry_date'     => $date,
...
function li_mi_erp_create_sale_journal( $sale ) {
	if ( ! $sale || empty( $sale->id ) ) {
		return 0;
	}

	$ar_account     = li_mi_erp_default_account( 'asset', '1003' );
	$income_account = li_mi_erp_default_account( 'income', '4001' );

	return li_mi_erp_create_journal_entry(
		$sale->sale_date,
		sprintf( 'Sale - %s (%s)', $sale->sale_no, li_mi_erp_contact_name( $sale->contact_id ) ),
		array(
...
function li_mi_erp_default_account( $type, $fallback_code ) {
	global $wpdb;
...
function li_mi_erp_sum( $rows, $key ) {
	$total = 0;
...
function li_mi_erp_get_account_balances_by_type() {
	global $wpdb;
	...
	foreach ( $accounts as $account ) {
		...
		$types[ $account->type ] += li_mi_erp_account_balance( $account->id );
	}
 ...
 */
function li_mi_erp_render_search_bar( $page_slug, $label, $placeholder, $hidden = array(), $current = '', $inline = false ) {
	...
	<form method="get" action="" class="<?php echo esc_attr( $form_class ); ?>">
		...
		<?php if ( $inline ) : ?>
			...
			<?php if ( $current ) : ?>
				<a href="<?php echo esc_url( li_mi_erp_admin_url( $page_slug, $hidden ) ); ?>" class="btn-secondary"><?php esc_html_e( 'Clear', 'lime-micro-erp' ); ?></a>
			<?php endif; ?>
		<?php else : ?>
 ...
 */
function li_mi_erp_render_pagination( $page_slug, $total_items, $per_page = 20 ) {
	...
	$paged       = li_mi_erp_query_int( 'paged', 1 );
	...
	foreach ( $filter_keys as $key ) {
		if ( li_mi_erp_query_has( $key ) && '' !== li_mi_erp_query_text( $key ) ) {
			$args[ $key ] = li_mi_erp_query_text( $key );
		}
	}

	$page_url = function ( $n ) use ( $page_slug, $args ) {
		return li_mi_erp_admin_url( $page_slug, array_merge( $args, array( 'paged' => $n > 1 ? $n : '' ) ) );
	};
